...
use griz_store::{OnStale, Operation, OperationState};
...
fn restore_since_spans_three_operations_byte_exact() -> TestResult {
    ...
    let restored = fx
        .store
        .restore_since(&op1.id, &[], OnStale::Refuse, "roll back")?;
    assert_eq!(
        restored.state,
...
fn a_broken_chain_refuses_the_whole_restore_unless_narrowed() -> TestResult {
    ...
    let narrowed =
        fx.store
            .restore_since(&op1.id, &[fx.path("b.rs")], OnStale::Refuse, "roll back b")?;
    assert_eq!(
        narrowed.state,
        OperationState::Applied,
        "{:?}",
...
fn a_broken_chain_names_the_file_and_offers_no_merge() -> TestResult {
    ...
    let merged = fx
        .store
        .restore_since(&op1.id, &[], OnStale::Merge, "roll back merged")?;
